...
int main(void) {
    size_t sizes[] = { 4*1024, 8*1024, 16*1024, 32*1024, 64*1024, 128*1024, 256*1024, 512*1024, 1024*1024, 4*1024*1024, 16*1024*1024, 64*1024*1024 };
    int n = sizeof(sizes)/sizeof(sizes[0]);
    printf("%-12s %10s %10s\n", "Size(KB)", "Seq(ns/B)", "Rand(ns/B)");
    for (int i = 0; i < n; i++) {
        printf("%-12zu %10.3f %10.3f\n", sizes[i]/1024, bench_seq(sizes[i]), bench_rand(sizes[i]));
    }
    return 0;
}